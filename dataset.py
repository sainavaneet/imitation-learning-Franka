...
def joint_states_callback(msg):
    ...
    current_joint_angles = np.array(msg.position[2:])
...
def main():
    ...
    waypoints_square = np.array([[0.5, 0.0, 0.3],[0.5, 0.0, 0.3],[0.5, -0.2, 0.3], [0.5, -0.2, 0.5], [0.5, 0.0, 0.5], [0.5, 0.0, 0.3], [0.5, 0.0, 0.3],
                                 [0.6, 0.2, 0.6], [0.6, 0.0, 0.6], [0.4, 0.0, 0.6], [0.4, 0.2, 0.6], [0.6, 0.2, 0.6],
                                 [0.5, 0.0, 0.3],[0.5, 0.0, 0.3],[0.5, -0.2, 0.3], [0.5, -0.2, 0.5], [0.5, 0.0, 0.5], [0.5, 0.0, 0.3], [0.5, 0.0, 0.3],
                                 [0.6, 0.2, 0.6], [0.6, 0.0, 0.6], [0.4, 0.0, 0.6], [0.4, 0.2, 0.6], [0.6, 0.2, 0.6]])  # Ensure waypoints are numpy arrays
        ...
        if solution is not None and not np.isnan(solution).any():
            ...
            if current_joint_angles is not None and current_end_effector_position is not None:
                dataset_entry = {
                    ...
                    "desired_end_effector_position": current_end_effector_position.tolist()
                }
                dataset.append(dataset_entry)
        else:
            rospy.logwarn("IK solution not found or invalid for waypoint: %s", waypoint)

    with open("/home/navaneet/Desktop/GITHUB/imitation-learning-Franka/datasets/dataset.json", "w") as json_file:
        json.dump(dataset, json_file)
    rospy.loginfo("Dataset saved to datasets/dataset.json")
